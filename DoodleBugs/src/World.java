...
public class World extends GraphicsProgram
{
	...
	public void run()
	{
		...
		{	
			for(int col = 0; col < GRID_SIZE_COL; col++)
			{
				...
				{
					bugs[row][col] = new DoodleBug(this, row, col);//this is World
				}
			}
		}
	...
	{
		resetSimulation();		//this avoids a bug being simulated more than once, check Organism
	...
	//simulates the specified type of bug - called from line 17 in Organism
		...
		for(int row = 0; row< GRID_SIZE_ROW; row++)
		{
			...
			for(int col = 0; col < GRID_SIZE_COL; col++)
			{
				...
				if(bug != null)
				{
					...
					GImage bImage = new GImage(bug.toString() + ".png", x, y);
					add(bImage);
				}
				x += 50;
			}
